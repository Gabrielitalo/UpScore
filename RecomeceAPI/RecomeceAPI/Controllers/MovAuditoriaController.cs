...
namespace RecomeceAPI.Controllers
{
  ...
  public class MovAuditoriaController : ControllerBase
  {
    private readonly MovAuditoriaService _service;
    private readonly HandleCatchService _handleCatchService;

    public MovAuditoriaController(MovAuditoriaService service, HandleCatchService handleCatchService)
    {
      _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Save([FromBody] MovAuditoriaModel model)
    {
      try
      {
    ...
    [HttpPut]
    public async Task<IActionResult> Update([FromBody] MovAuditoriaModel model)
    {
      try
      {

...
namespace RecomeceAPI.Controllers
{
  ...
  public class CadProdutosFaixasPagamentoController : ControllerBase
  {
    private readonly CadProdutosFaixasPagamentoService _service;
    private readonly HandleCatchService _handleCatchService;

    public CadProdutosFaixasPagamentoController(CadProdutosFaixasPagamentoService service, HandleCatchService handleCatchService)
    {
      _service = service;
    ...
    public async Task<IActionResult> Save([FromBody] object json)
    {
      try
      {
        var model = new CadProdutosFaixasPagamentoModel();
